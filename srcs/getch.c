...
void	getch(mlx_key_data_t key, void *param)
{
	...
	if (key.key == MLX_KEY_ESCAPE && key.action == MLX_PRESS)
	{
		...
		&& game->map[game->player_pos.y
			- 1][game->player_pos.x] != '1')
		move_north(game);
		...
		&& game->map[game->player_pos.y][game->player_pos.x
		+ 1] != '1')
		move_east(game);
...
void	move_north(t_game *game)
{
	...
	{
		mlx_image_to_window(game->mlx, game->image.exit, game->player_pos.x
			* 50, game->player_pos.y * 50);
		game->player_pos.y--;
	}
	else
	{
		mlx_image_to_window(game->mlx, game->image.floor,
			game->player_pos.x * 50, game->player_pos.y * 50);
		game->player_pos.y--;
	}
	if (game->map[game->player_pos.y][game->player_pos.x] == 'C')
	{
		game->num_coins--;
		mlx_image_to_window(game->mlx, game->image.floor,
			game->player_pos.x * 50, game->player_pos.y * 50);
	...
	mlx_image_to_window(game->mlx, game->image.player, game->player_pos.x
		* 50, game->player_pos.y * 50);
	game->move_count++;
